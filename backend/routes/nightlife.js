    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            for (const img of imagesFiles) {
                await pool.query(
                    "INSERT INTO nightlife_hotel_images (hotel_id, image_url) VALUES (?, ?)",
                    [hotelId, img]
                );
            }
...
router.get('/', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            ...
            FROM nightlife_hotels h
            LEFT JOIN nightlife_hotel_images i ON h.id = i.hotel_id
        `);
        ...
        console.error("❌ Error fetching nightlife hotels:", err.message);
...
router.get('/:slug', async (req, res) => {
    ...
    try {
        const [hotels] = await pool.query("SELECT * FROM nightlife_hotels");
        ...
        const [images] = await pool.query(
            "SELECT image_url FROM nightlife_hotel_images WHERE hotel_id = ?",
            [matchedHotel.id]
        );
