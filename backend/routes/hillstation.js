    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const [hotelResult] = await pool.query(
                `INSERT INTO hillstation_hotels
            ...
            for (const img of imagesFiles) {
                await pool.query(
                    "INSERT INTO hillstation_hotel_images (hotel_id, image_url) VALUES (?, ?)",
                    [hotelId, img]
                );
            }
...
router.get('/', async (req, res) => {
    try {
        ...
        console.error("❌ Error fetching hillstation hotels:", err.message);
...
router.get('/:slug', async (req, res) => {
    ...
    try {
        const [hotels] = await pool.query("SELECT * FROM hillstation_hotels");
        ...
        const [images] = await pool.query(
            "SELECT image_url FROM hillstation_hotel_images WHERE hotel_id = ?",
            [matchedHotel.id]
        );
