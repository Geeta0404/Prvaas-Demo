    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            for (const img of imagesFiles) {
                await pool.query(
                    "INSERT INTO adventure_hotel_images (hotel_id, image_url) VALUES (?, ?)",
                    [hotelId, img]
                );
            }
...
router.get('/', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            ...
            FROM adventure_hotels h
            LEFT JOIN adventure_hotel_images i ON h.id = i.hotel_id
        `);
        ...
        console.error("❌ Error fetching adventure hotels:", err.message);
...
router.get('/:slug', async (req, res) => {
    ...
    try {
        const [hotels] = await pool.query("SELECT * FROM adventure_hotels");
        ...
        const [images] = await pool.query(
            "SELECT image_url FROM adventure_hotel_images WHERE hotel_id = ?",
            [matchedHotel.id]
        );
