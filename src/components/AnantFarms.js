...
export default function RoadtripHotels() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/roadtrip");
                  ...
                  <div className="row g-2">
                    {hotel.images && hotel.images.length > 0 ? (
                      ...
                      [...Array(6)].map((_, imgIndex) => (
                        <div className="col-4" key={imgIndex}>
                          <img
                            src={`https://source.unsplash.com/600x400/?roadtrip,travel,${imgIndex}`}
