...
export default function Adventure() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/adventure");
        setRoadTripData(res.data);
      } catch (err) {
    ...
    <>
      ...
      <div style={{ backgroundColor: "rgb(240, 240, 240)" }}>
        <div className="py-5 container">
          ...
          {roadTripData.length === 0 && !error ? (
            <p>Loading...</p>
          ) : (
            <div className="d-flex flex-column gap-5">
              {roadTripData.map((hotel, index) => (
                <div
                  ...
                  <div className="col-md-5 resort-card-modern text-center">
                    <div className="p-3">
                      ...
                      <button
                        ...
                        onClick={() => navigate(`/resort/${slugify(hotel.name)}`)}
                      >
                        Discover More
                      </button>
                    </div>
                    ...
                    <div className="row g-2">
                      {hotel.images && hotel.images.length > 0 ? (
                        ...
                        [...Array(6)].map((_, imgIndex) => (
                          <div className="col-4" key={imgIndex}>
                            <img
                              src={`https://source.unsplash.com/600x400/?resort,nature,${imgIndex}`}
                              alt={`fallback-roadtrip-${imgIndex}`}
